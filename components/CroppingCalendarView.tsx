    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2 border-b border-gray-200">
          {SEASONS.map(season => (
            <button
              ...
              className={`px-3 md:px-6 py-2 md:py-3 font-semibold transition-all border-b-2 ${
      ...
      {isFormOpen && editForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] md:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="card-gov overflow-hidden">
          <div className="overflow-x-auto">
            <table className="table-gov w-full">
              <thead>
                <tr>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-left">Month</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-left">Crop</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-left">Activity</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-left">Notes</th>
              ...
              <tbody>
                {filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={isAdmin ? 5 : 4} className="px-3 md:px-6 py-12 text-center text-gray-500">
                  ...
                  filteredData
                    ...
                    .map(activity => (
                      <tr key={activity.id} className="hover:bg-brand-50/50 transition-colors">
                        <td className="px-3 md:px-6 py-3 md:py-4 font-medium text-gray-900">{activity.month}</td>
                        <td className="px-3 md:px-6 py-3 md:py-4 text-gray-700">{activity.crop}</td>
                        <td className="px-3 md:px-6 py-3 md:py-4 text-gray-700">{activity.activity}</td>
                        <td className="px-3 md:px-6 py-3 md:py-4 text-gray-600 text-sm">{activity.notes || '—'}</td>
                        {isAdmin && (
