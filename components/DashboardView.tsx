    ...
    <div className="space-y-5 md:space-y-8">
      ...
      {totalAlerts > 0 && (
        <div className="card-gov p-0 overflow-hidden animate-slide-down">
          <div className="bg-gradient-to-r from-red-600 via-red-500 to-amber-500 px-4 md:px-6 py-3 flex items-center justify-between">
            <div className="flex items-center space-x-2 md:space-x-3">
              <AlertTriangle size={16} className="text-white flex-shrink-0" />
              <h3 className="font-heading font-extrabold text-white text-xs md:text-sm tracking-wide">{totalAlerts} Active Alert{totalAlerts > 1 ? 's' : ''}</h3>
            </div>
            <span className="text-[8px] md:text-[9px] font-heading font-bold text-white/70 uppercase tracking-widest hidden sm:block">Requires Attention</span>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-700">

            {/* Stock Reorder Alerts */}
            {stockAlerts.map(item => (
              <div key={item.itemCode} className="flex items-center justify-between px-3 md:px-6 py-2.5 md:py-3 hover:bg-red-50/50 dark:hover:bg-red-900/10 transition-colors cursor-pointer" onClick={() => setActiveTab('Store & Stock')}>
                <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-1">
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-red-100 dark:bg-red-900/30 flex items-center justify-center flex-shrink-0">
                    <ShoppingCart size={13} className="text-red-600 dark:text-red-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="font-heading font-bold text-slate-800 dark:text-white text-xs truncate">{item.itemName}</p>
                    <p className="text-[10px] text-slate-400 font-body truncate">{item.itemCode} - Stock Reorder</p>
                  </div>
                </div>
            ...
            {vehicleAlerts.map(l => (
              <div key={l.id} className="flex items-center justify-between px-3 md:px-6 py-2.5 md:py-3 hover:bg-amber-50/50 dark:hover:bg-amber-900/10 transition-colors cursor-pointer" onClick={() => setActiveTab('Vehicle Maint.')}>
                <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-1">
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center flex-shrink-0">
                    <Wrench size={13} className="text-amber-600 dark:text-amber-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="font-heading font-bold text-slate-800 dark:text-white text-xs truncate">{l.vehicleLabel}</p>
                    <p className="text-[10px] text-slate-400 font-body truncate">{l.type} - Service Due</p>
                  </div>
                </div>
                <div className="text-right flex-shrink-0 ml-2">
            ...
            {incrementAlerts.map(emp => (
              <div key={emp.empId} className="flex items-center justify-between px-3 md:px-6 py-2.5 md:py-3 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors cursor-pointer" onClick={() => setActiveTab('HR Management')}>
                <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-1">
                  {emp.profilePic ? (
                    <img src={emp.profilePic} alt="" className="w-7 h-7 md:w-8 md:h-8 rounded-lg object-cover border border-slate-200 dark:border-slate-600 flex-shrink-0" />
                  ) : (
                    <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0">
                      <CalendarClock size={13} className="text-blue-600 dark:text-blue-400" />
                    </div>
                  )}
                  <div className="min-w-0">
                    <p className="font-heading font-bold text-slate-800 dark:text-white text-xs truncate">{emp.name}</p>
                    <p className="text-[10px] text-slate-400 font-body truncate">{emp.empId} - Salary Increment</p>
                  </div>
                </div>
                <div className="text-right flex-shrink-0 ml-2">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <div className="card-gov p-3 md:p-5">
            <p className="text-slate-400 text-[9px] md:text-[10px] font-heading font-semibold uppercase tracking-wider mb-1">Active Vehicles</p>
            <h3 className="text-lg md:text-2xl font-heading font-extrabold text-slate-800 dark:text-white">{vehicleStats.activeVehicles} <span className="text-xs md:text-sm font-normal text-slate-300">/ {data.vehicles.length}</span></h3>
          ...
          <div className="card-gov p-3 md:p-5">
            <p className="text-slate-400 text-[9px] md:text-[10px] font-heading font-semibold uppercase tracking-wider mb-1">Total Fuel Cost</p>
            <h3 className="text-lg md:text-2xl font-heading font-extrabold text-slate-800 dark:text-white">Rs. {fmt(vehicleStats.totalFuelCost)}</h3>
            <p className="text-[9px] md:text-[10px] text-slate-400 mt-1 font-body">{fmt(vehicleStats.totalFuelLiters)} Liters used</p>
          </div>
          <div className="card-gov p-3 md:p-5">
            <p className="text-slate-400 text-[9px] md:text-[10px] font-heading font-semibold uppercase tracking-wider mb-1">Maintenance Cost</p>
            <h3 className="text-lg md:text-2xl font-heading font-extrabold text-slate-800 dark:text-white">Rs. {fmt(vehicleStats.totalMaintCost)}</h3>
            <p className="text-[9px] md:text-[10px] text-slate-400 mt-1 font-body">+ Rs. {fmt(vehicleStats.totalRepairCost)} repairs</p>
          </div>
          <div className="card-gov p-3 md:p-5">
            <p className="text-slate-400 text-[9px] md:text-[10px] font-heading font-semibold uppercase tracking-wider mb-1">Total Distance</p>
            <h3 className="text-lg md:text-2xl font-heading font-extrabold text-slate-800 dark:text-white">{fmt(vehicleStats.totalDistance)} km</h3>
            <p className="text-[9px] md:text-[10px] text-slate-400 mt-1 font-body">{data.dailyVehicleLogs.length} trips logged</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <div className="card-gov p-3 md:p-5">
            <p className="text-slate-400 text-[9px] md:text-[10px] font-heading font-semibold uppercase tracking-wider mb-1">Total Employees</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div className="card-gov p-4 md:p-6">
        ...
        <div className="card-gov p-4 md:p-6">
          ...
          {stockByCategory.length > 0 ? (
            ...
            <div className="h-40 md:h-52 flex items-center justify-center text-slate-400 text-sm font-body">No stock data yet</div>
          )}
        </div>
      </div>
      ...
      {data.vehicles.length > 0 && (
        <div className="card-gov p-4 md:p-6">
          <h4 className="font-heading font-bold text-slate-800 dark:text-white flex items-center mb-4 md:mb-5 text-sm md:text-base">
            <Activity size={16} className="mr-2 text-brand-500" /> Fleet Status
          </h4>
