    ...
    <>
      {showForm && isAdmin && (
        <div className="bg-white p-8 rounded-3xl border-2 border-blue-50 shadow-xl space-y-8 animate-fade-in">
          ...
          <form onSubmit={handleSubmitItem} className="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
            <tr>
              <th className="px-6 py-4">SB Page No</th>
            ...
            {stockItems.length === 0 ? (
              ...
              stockItems.map(item => (
                <tr key={item.itemCode} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-slate-800">{item.openingStock}</td>
                  <td className="px-6 py-4 text-right text-slate-600">Rs. {fmt(item.unitPrice)}</td>
    ...
    <>
      {showForm && isAdmin && (
        <div className="bg-white p-8 rounded-3xl border-2 border-blue-50 shadow-xl space-y-8 animate-fade-in">
          ...
          <form onSubmit={handleSubmitStockIn} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-1.5">
              ...
              [...stockInRecords].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(record => {
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4 text-slate-500">{new Date(record.date).toLocaleDateString('en-GB')}</td>
                    <td className="px-6 py-4">
                      <span className="font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{record.itemCode}</span>
                    </td>
                    <td className="px-6 py-4 text-slate-700">{item?.itemName || '-'}</td>
                    <td className="px-6 py-4 text-right font-extrabold text-slate-800">{record.qty}</td>
                    <td className="px-6 py-4 text-slate-500">{record.supplier}</td>
                    <td className="px-6 py-4 text-slate-500">{record.grnNo}</td>
                    {isAdmin && (
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <>
      {showForm && isAdmin && (
        <div className="bg-white p-8 rounded-3xl border-2 border-blue-50 shadow-xl space-y-8 animate-fade-in">
          ...
          <form onSubmit={handleSubmitStockOut} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-1.5">
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Date</label>
              ...
              [...stockOutRecords].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(record => {
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-slate-700">{item?.itemName || '-'}</td>
                    <td className="px-6 py-4 text-right font-extrabold text-slate-800">{record.qty}</td>
                    <td className="px-6 py-4 text-slate-500">{record.issuedTo}</td>
                    <td className="px-6 py-4 text-slate-500">{record.purpose}</td>
                    {isAdmin && (
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
            <tr>
              <th className="px-4 py-4">SB Page No</th>
              <th className="px-4 py-4">Product Name</th>
              <th className="px-4 py-4 text-right">Opening</th>
              <th className="px-4 py-4 text-right">Total In</th>
              <th className="px-4 py-4 text-right">Total Out</th>
              ...
              <th className="px-4 py-4 text-right">Value (LKR)</th>
              <th className="px-4 py-4 text-center">Procurement</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 font-medium">
