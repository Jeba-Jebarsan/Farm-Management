...
const VehicleMaintenanceView: React.FC<Props> = ({
  ...
  const renderVehicleRegister = () => (
    <div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-800 p-3 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-700 mb-6 shadow-sm">
          ...
          <div className="flex space-x-3 mt-4">
            <button onClick={() => wrap(async () => isEditing ? await onUpdateVehicle(vehicleForm) : await onAddVehicle(vehicleForm))}
              ...
              className="bg-green-600 text-white px-3 md:px-6 py-2 rounded-lg text-sm font-bold hover:bg-green-700 disabled:opacity-50 flex items-center space-x-2">
              {saving && <Loader2 className="animate-spin" size={14} />}
              <span>{isEditing ? 'Update' : 'Save'}</span>
            </button>
            <button onClick={resetForm} className="px-3 md:px-6 py-2 rounded-lg border border-slate-200 dark:border-slate-600 text-sm font-bold text-slate-600 dark:text-slate-300">Cancel</button>
          </div>
        </div>
      )}
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-600">
                {['Vehicle No', 'Plate No', 'Type', 'Model', 'Year', 'Engine No', 'Status', 'Joined Date', 'Renewal Date', ...(isAdmin ? ['Actions'] : [])].map(h =>
                  <th key={h} className="px-2 md:px-4 py-2 md:py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{h}</th>
                )}
              </tr>
            </thead>
