    ...
    <div className="space-y-6">
      ...
      {showForm && isAdmin && (
        <div className="bg-white p-4 md:p-8 rounded-3xl border-2 border-blue-50 shadow-xl space-y-6 md:space-y-8 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
            <tr>
              ...
              <th className="px-3 md:px-6 py-3 md:py-4">Plate No</th>
              <th className="px-3 md:px-6 py-3 md:py-4">Transaction Date</th>
              <th className="px-3 md:px-6 py-3 md:py-4">Technical Details</th>
              <th className="px-3 md:px-6 py-3 md:py-4">Cost (LKR)</th>
              {isAdmin && <th className="px-3 md:px-6 py-3 md:py-4 text-right">Actions</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 font-medium">
            {data.length === 0 ? (
              <tr><td colSpan={isAdmin ? 6 : 5} className="px-3 md:px-6 py-16 text-center">
              ...
              [...data].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((log) => (
                <tr key={log.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-3 md:px-6 py-3 md:py-4 font-mono font-bold text-slate-500">
                    {log.legalPlateNo}
                  </td>
                  <td className="px-3 md:px-6 py-3 md:py-4 text-slate-500">
                    {new Date(log.date).toLocaleDateString('en-GB')}
                  </td>
                  <td className="px-3 md:px-6 py-3 md:py-4 text-slate-700">
                    {isMaintenance ? log.description : isRepair ? log.issue : isInsurance ? `${log.policyNo} (${log.company})` : `${log.quantity}L @ ${log.supplier}`}
                  </td>
                  <td className="px-3 md:px-6 py-3 md:py-4 font-extrabold text-slate-800">
